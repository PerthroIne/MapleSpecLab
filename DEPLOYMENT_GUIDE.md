...
3. `Summary`에 `Release v1.1.0 - official guide integration`을 입력합니다.
4. `Description`에 다음을 입력합니다.

   `공식 가이드 설명 UI, 비교 모델 한계 안내, 모바일 접기/펼치기 추가`
...
2. 브라우저에 이전 화면이 남으면 강력 새로고침하거나 주소 뒤에 `?v=1.1.0`을 붙여 확인합니다.
3. 정보 메뉴에서 `v1.1.0`과 공식 가이드 확인일이 보이는지 확인합니다.
4. 현재 스펙의 `ⓘ`, 도움말의 공식 기준 카드, 동료 등급 접기/펼치기를 확인합니다.

## 6. GitHub Release 만들기

1. 저장소 웹페이지에서 `Releases` → `Draft a new release`를 엽니다.
2. `Choose a tag`에 `v1.1.0`을 입력하고 새 태그를 만듭니다.
3. 제목은 `MapleSpecLab v1.1.0`으로 입력합니다.
4. 본문에는 `CHANGELOG.md`의 v1.1.0 항목을 붙여넣습니다.
